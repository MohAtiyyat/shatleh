        ...
        <div class="form-container">
            <form id="productForm" enctype="multipart/form-data" method="POST"
                ...
                <div class="form-group">
                            ...
                            @foreach ($item->image as $index => $image)
                                <div class="image-preview" data-index="{{ $index }}">
                                    ...
                                    <button type="button" class="delete-btn"
                                        onclick="deleteImage({{ $index }})">×</button>
                                    <p>{{ basename($image) }}</p>
                    ...
                    <div class="custom-file">
                        <input type="file" class="custom-file-input" id="images" name="images[]"
                            accept="image/jpeg,image/png,image/jpg" multiple {{ isset($item) ? '' : 'required' }}>
                ...
                <div class="form-group">
                    ...
                    <textarea id="description_ar" name="description_ar" placeholder="Enter Arabic description" rows="4" required
                        dir="rtl">{{ old('description_ar', isset($item) ? $item->description_ar : '') }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" required>
                        <option value="active"
                            {{ old('status', isset($item) ? $item->status : '') == '1' ? 'selected' : '' }}>Active</option>
                        <option value="inactive"
                            {{ old('status', isset($item) ? $item->status : '') == '0' ? 'selected' : '' }}>Inactive
                        </option>
                    </select>
                    @error('status')
                        <div class="text-danger">{{ $message }}</div>
                ...
                <div class="form-group">
                    ...
                    <select id="availability" name="availability" required>
                        <option value="1"
                            {{ old('availability', isset($item) ? $item->availability : '') == '1' ? 'selected' : '' }}>In
                            Stock</option>
                        <option value="0"
                            {{ old('availability', isset($item) ? $item->availability : '') == '0' ? 'selected' : '' }}>Out
                            of Stock</option>
                    </select>
                    @error('availability')
                        <div class="text-danger">{{ $message }}</div>
            ...
            window.deleteImage = function(index) {
                if (confirm('Are you sure you want to delete this image?')) {
                    ...
                    const hiddenInput = document.querySelector(
                        `input[name="existing_images[]"][data-index="${index}"]`);
                    if (container) {
                        container.remove();
                    }
                    if (hiddenInput) {
